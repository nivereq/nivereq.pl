    ...
    return (
      <div className="status spotifyStatus anim-appearing">
        <a href={"https://open.spotify.com/track/" + status.spotify.track_id}>
          <span>
            <i style={{ color: "#1ed760" }} className="bi bi-spotify" />
          </span>
          <span className="artistName">{status.spotify.artist}</span> -{" "}
          <span className="songName">{status.spotify.song}</span>
        </a>
      </div>
    );
  } else {
